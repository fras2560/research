...
\end{Lemma}
\noindent {\it Proof}.  Expanding \cite{BraFud2002} upon Theorem 2 Claim 1 where it was shown $C_l$, $l \geq 7$ has no $k$-vertex for $k \geq 1$. If the set of 0-vertex $= \phi$ then $G$ is a $C_l$ else it is a disconnected graph.  $\Box$
...
\end{itemize}
\end{Lemma}
\noindent {\it Proof.} Lemma \ref{lem:C7Cycle} showed all $G$ containing a $C_l$ has no $k$-vertex for $k \in {1, 2, 3, 4, 5, 6}$. So $\overline{C_l}$ has no $k$-vertex for $k \in {0, 1, 2, 3, 4, 5,}$ by Lemma \ref{lem:complement-k-vertex} . Let $H$ be the set of 6-vertex. So if $H = \phi$ then $G$ is a $\overline{C_l}$ else $G$ is a join of $\overline{C_l}$ and $H$.
