...
\end{Lemma}
\noindent {\it Proof.} Suppose $|X_i| \geq 2$ with vertices $x_1$ and $x_2$. Let $x_j$ be a vertex from $X_{i+1} \cup X_{i+2}$. If $x_j \; \circled{1} \; X_i$ then there is a $\overline{Twin-House}$.
...
\end{Lemma}
\noindent {\it Proof.} If $|R| \geq 3$ then by Lemma \ref{lem:xi-limits-r} is a clique cutset. If $|R| \leq 2$ then by Lemmas \ref{lem:r-limits-xi} and \ref{lem:2xi-cojoins-xj} $|X_i| \leq,\; i \in {0,1,2,3,4}$. By Lemma \ref{lem:c5-4vertex-bounded} the number of $4$-vertex$ \leq 5$. Then $|V| \leq 2 + 5 + 5 + 5$ or $|R| \geq 3$ and $G$ contains a clique cutset.
