...
\end{Lemma}
\noindent {\it Proof.} By Lemma \ref{lem:Xi-Yi-noYi} at least one $Y_i = \phi$, assume $Y_3 = \phi$ and we are adding a vertex $k \in X_0$. There are six distinct cases two deal with.
...
(III) If the maximal clique $\subset Y_0 \cup Y_1 \cup 0 \cup 1$. Adding $k$ then will result in $\omeag(G)$ to remain the same. Let $A$ denote the set of vertices from $Y_0 \cup Y_1$. $|A| > |Y_0|$ and $|A| > |Y_1|$ else it is Case I or Case II. 
