...
In section~\ref{sec:definitions}, we give definitions and background on our problem. In section~\ref{sec:updates}, we expand upon previous Lemmas \cite{BraFud2002}. In section!\ref{sec:characterization} , we give the proofs or our main results.  In section~\ref{sec:5-critical}, we demostrate how  
...
\begin{Theorem}\label{thm:StrongPerfect}{\rm \cite{ChvHoa1987}}
...
%
In this section, we prove 3 properties of {\CCO}-free graphs and we show that $k$-critical {\CCO}-free graph is finite for some fixed $k$ where $k \geq 3$
%
...
\end{Lemma}
\noindent {\it Proof}.  Expanding \cite{BraFud2002} upon Theorem 2 Claim 1 where it was shown $C_l$, $l \geq 7$ has not $k$-vertex for $k \geq 1$. If the set of 0-vertex $= \phi$ then $G$ is a $C_l$ else it is a disconnected graph.  $\Box$
