...
induced subgraphs is ($k-1$)-colorable. There is an infinite number of k-critical {\CCO} for each $k \geq 3$. We show that the set of $k$-critical {\CCO} for some $k \geq 3$ is a clique $K_k$ or the join of $k-2$ set with a $C_5$  
...
$k$-critical {\CCO}-free graphs are constructed for $k \leq 5$.
...
\end{Lemma}
\noindent{\it Proof.} Let $C$ be a $C_6$ with vertices 1,2,...,6. note that $C$ has no $k$-vertex for $k \in {1,2,3,5,6}$ since $G$ is $claw$-- and $co$-$claw$-free, and $N_1,_2,_4,_5$, $N_2,_3,_5,_6$, $N_3,_4,_6,_1$ is a partition of the set of 4-vertices since $G$ is $co$-$claw$-free. Since $G$ is $claw$-free, there are no edges between these sets, and there are no 0-vertices to a 4-vertex whichs means that these sets are modules and thus have at most one vertex. Now $G$ has at most 9 vertices. $\Box$
...
\end{itemize}
\end{Lemma}
\noindent{\it Proof.} Let $C$ be a $C-5$ with vertices 1,2,...,5. Note that $C$ has no $k$-vertex for $k \in {1, 2, 3, 4}$ since $G$ is $claw$- and $co$-$claw$-free, and $C$ has no 0-vertex adjacent to a 5-vertex since $G$ is $claw$-free which means that the set of 5-vertices is a complete to $C$. If the set of 5-vertices is empty then $G$ is a $C$ or a join of $C$ and $H$. $\Box$
...
If $G$ contains a $P_l,$ $l \geq 6$, then $G$ is such a path
\end{Lemma}
{\it Proof.} Let $P$ be a longest induced path $pl,$ $l \geq 6$, in $G$ with vertives 1,2,...,$l$.
First we show that $P$ has no 1- and no 2-vertex: Since $P$ is a longest induced path, $P$ has no 1-vertex adjacent to 1 or to $l$, and since $G$ is $claw$-free, $P$ has no 1-vertex to a vertex $u \in {2,3,...,l-1}$
Since $G$ is $co$-$claw$-free, the two neighbors of a 2-vertex for $P$ are non-consecutive in $P$. If one of them is different from 1 or $L$ then there is a $claw$. THus the only possibility is that a 2-vertex is adjacenet to 1 and $l$, but would then $G$ would be a $C_k+1$, $k+1 \geq 5$. Thus $P$ has no 2-vertex.
Assume now that $x$ is a $k$-vertex, $k \geq 3$, for $P$. Since $G$ is $claw$-free, at least two of the neighbours of $x$ in $P$ are consecutive in $P$. If $x$ is adjacent to 1 and 2 then $xi \in E$ for all $i \geq 4$, else 12$xi$ is a $co$-$claw$ but now $x$146 is a $claw-$ contradiction
If $x$ is adjacent to 2 and 3 and nonadjacent to 1 then $xi \in E$ for all $i \geq 5$ for the same reasons but now $x$561 is a $co$-$claw-$contradiction.
If x is nonadjacent to 1 and 2 and adjacent to $i$ and $i$ + 1 then $x$, $i$, $i$+ 1, 1 is a $co$-$claw$ for consecutive neighbors $i$, $i$ + 1 of $x$. NOte that there are no 0-vertices for $P$. $\Box$

\begin{Observation}\label{obs:Assumption2}
...
If $G$ contains a $P5$ then $G$ has at most 8 vertices.
\end{Lemma}
\noindent {\it Proof.} Assume now that $x$ is a $k$-vertex for $P$ with $k \geq 3$. Since $G$ is $claw$-free, at least two of the neighbors of $x$ are consecutive in $P$. If $x$ is adjacent to 1 and 2 then $x$ is adjacent to 4 and 5 and nonadjacent to 3. If $x$ is adjacent to 2 and 3 but noadjacent to 1 then $x$ is adjacent to 5 and nonadjacent to 4, and symmetricallt for x being adjacent to 3 and 4 and nonadjacent to 5. Thus, the only possible $k$-vertices for $P$ with $k \geq 3$ are the sets $A = N_1,_2,_4,_5$, $B = N_1,_3,_4$,_ and $C = N_2,_3,_5$. Moreover since $G$ is $claw$-free, there are no edges between $A$ and $B$ ($claw-$ $AB$, $A$2, $A$5), between $A$ and $C$ ($claw-$$AC$, $A$1, $A$4), and between $B$ and $C$ ($claw$- $BC$, $B$1, $B4$). Moreever $P$ has no 0-vertices since G is $claw-free$. $A$, $B$, $C$ can have at most one vertex since $G$ is $claw$-free. Thus, $G$ can have at most 9 vertices.
